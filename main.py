...
def array_cleaner(independent_var, dependent_var, start, stop, length):  # could try some sort of log system here but still thinking
...
while degree_count <= 359.0:
    ...
    if igbt_current >= 0.0:
        ...
        igbt_cond_losses = igbt_current * time_division * vce_at_ic * output_voltage[0] / input_bus_voltage / 1000.0
        # e_sw_on = esw_solver(ic_from_e_sw_on_125, e_sw_on_from_e_sw_on_125, ic_from_e_sw_on_150, e_sw_on_from_e_sw_on_150, tj_try_igbt, igbt_current, 2400)
        e_sw_on_curve = sp.interp1d(ic_from_e_sw_on_125, e_sw_on_from_e_sw_on_125)
        e_sw_on = e_sw_on_curve(igbt_current)
        ...
        e_sw_off_curve = sp.interp1d(ic_from_e_sw_off_125, e_sw_off_from_e_sw_off_125, kind='linear')
        ESwOff = e_sw_off_curve(igbt_current)
        if 1.0 > output_voltage[0] / input_bus_voltage > 0.0:
            e_sw_on_per_cycle = switches_per_cycle_per_degree * e_sw_on
    ...
    # e_sw_onFromRge_sw_on = esw_solver(e_rg_from_e_on_125, e_on_from_e_on_125, e_rg_from_e_on_150, e_on_from_e_on_150, tj_try_igbt, input_rg_on, 10)
    ...
    p_igbt_cond.append(igbt_cond_losses * freq_output / 1000.0)
    e_sw_on.append(e_sw_on_per_cycle * freq_output / 1000.0 * vcc_ratio * e_sw_on_from_rg_on)
    e_sw_off.append(dtTr1EoffFcoRatio * freq_output / 1000.0 * vcc_ratio * e_sw_off_from_rg_off)
    e_sw_on_Eoff.append((e_sw_on_per_cycle * vcc_ratio * e_sw_on_from_rg_on + dtTr1EoffFcoRatio * vcc_ratio * e_sw_off_from_rg_off) * freq_output / 1000.0)
    p_fwd_cond.append(dtDi1Pvce * freq_output / 1000.0)
    e_sw_err.append(dtDi1ErrFcoRatio * freq_output / 1000.0 * vcc_ratio * errFromRgErr)
    p_inst_tr.append((p_igbt_cond[degree_count] + (e_sw_on[degree_count] + e_sw_off[degree_count]) * (input_bus_voltage / vcc_value)) / time_division * 1000.0)
    p_inst_di.append((p_fwd_cond[degree_count] + e_sw_err[degree_count] * (input_bus_voltage / vcc_value)) / time_division * 1000.0)
    p_igbt.append((e_sw_on_per_cycle * vcc_ratio * e_sw_on_from_rg_on + dtTr1EoffFcoRatio * vcc_ratio * e_sw_off_from_rg_off) * freq_output / 1000.0 + igbt_cond_losses * freq_output / 1000.0)
    p_fwd.append(dtDi1ErrFcoRatio * errFromRgErr * freq_output / 1000.0 * vcc_ratio + dtDi1Pvce * freq_output / 1000.0)
    p_arm.append(2 * ((e_sw_on_per_cycle * vcc_ratio * e_sw_on_from_rg_on + dtTr1EoffFcoRatio * vcc_ratio * e_sw_off_from_rg_off) * freq_output / 1000.0 + igbt_cond_losses * freq_output / 1000.0 + (
            dtDi1ErrFcoRatio * errFromRgErr * freq_output / 1000.0 * vcc_ratio + dtDi1Pvce * freq_output / 1000.0)))
...
print('Delta TC ' + str(delta_tc))
print('P_arm = ' + str(np.sum(p_arm)))
print('Delta Tj IGBT is ' + str(delta_tj__igbt))
print('Tj settled at ' + str(tj_igbt))
print('P cond IGBT is ' + str(p_igbt_cond_total))
print('e_sw_on losses are ' + str(e_sw_on_total))
print('ESwOff losses are ' + str(e_sw_off_total))
print('Total IGBT losses ' + str(p_igbt_total))
print('Delta Tj diode is ' + str(delta_tj__fwd))
